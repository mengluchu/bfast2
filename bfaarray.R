...
#  hpc='none'
 # level=0.05
#  type = "OLS-MOSUM"
  bfa <- try(bfastmore(ts, h = h, season = season, 
                 max.iter =max.iter ,
   ...
   if (LBP < breaks) # just to avoid array with variables of different length
   {
       bkpt[(LBP+1):breaks]<-0
       magn[(LBP+1):breaks]<-0
       bclass[(LBP+1):breaks]<-0
   }
   res <- c(as.vector(bkpt),magn, bclass )
   names(res) <- c(rep("breakpoint", length(bkpt)) , rep("mag",length(magn)),rep("bclass",length(bclass)))
   return(res)
  }
